...
export async function runSetupWizard(): Promise<void> {
  ...
  const { optionalFeatures } = await inquirer.prompt([{
    ...
    choices: [
      ...
      { name: '🐦 Twitter/X — she can post tweets autonomously (needs Twitter API keys)', value: 'twitter' },
  ...
  if (optionalFeatures.includes('twitter')) {
    ...
    if (twitterAnswers.clientId) {
      ...
      envValues.SOCIAL_AUTO_POST = 'false'
    }

    console.log(chalk.cyan(isCN
      ? '\n  ℹ️  设置完成后，运行 `node test-twitter-oauth2.mjs` 完成授权'
      : '\n  ℹ️  After setup, run `node test-twitter-oauth2.mjs` to complete authorization'))
  }
